...
/-- Order on the model is CMRA inclusion. -/
scoped instance : LE (Model I α V F) := ⟨fun x y => CMRA.Included x y⟩
  ...
  «exists» (fun h : ∀ i, ProbabilityTheory.ProbabilitySpace.compatiblePerm (P i) (p i) =>
    own (M := Model I α V F)
      (fun i => ⟨⟨WithTop.some (P i), p i⟩,
        by
          -- coerce ProbabilitySpace-level compatibility to PSp-level
          simpa [ProbabilityTheory.ProbabilitySpace.compatiblePerm] using (h i)⟩))
    ...
    «exists» (fun compatP : {P : I → ProbabilityTheory.ProbabilitySpace (α → V) // ∀ i, ProbabilityTheory.ProbabilitySpace.compatiblePerm (P i) (p i)} =>
      own (M := Model I α V F)
        (fun i => ⟨⟨WithTop.some (compatP.1 i), p i⟩,
          by
            simpa [ProbabilityTheory.ProbabilitySpace.compatiblePerm] using (compatP.2 i)⟩))
