...
def ownPSp (P : I → PSp (Var → Val)) : bProp I Var Val :=
  ∃' (p : IxCompatiblePermission P), ownIndexedPSpPm P p
...
/-
F sigma-algebra over B (may not be discrete)
F' sigma-algebra over C (may not be discrete)
μ : D(Σ_A)
κ₁ : A → D(F)
...
bind(bind(μ, κ₁), κ₂)
               [n] -> MI -> Prop
𝒞_{unif(n)} (λ v. λ (F, μ, p). if even(v.val) then μ 0 = 1 else μ v = 1)
...
wp P {v. Pr[v = 0] = 1/2}
    ...
    {T} P {v. Pr[v = 0] = 1/2 ∧ Pr[v odd] = 1/2}
<=> T => wp P (v. Pr[v = 0] = 1/2 ∧ Pr[v odd] = 1/2)
-/
...
def cond {A Var Val : Type*}
  ...
  ∃' (κ : CompatibleKernel A m),
    own m.val
      ...
      ∧' (∀' (v : μ.support), ↑ ((fun i =>
        ⟨⟨some ⟨⟨m.ms i, κ.kernel i v⟩, κ.isProb i v⟩, m.perm i⟩, κ.isComp i v⟩) ∈ (K v).carrier))
...
theorem ESureDirac
  ...
  constructor
  · intro m hv h
    obtain ⟨P, b₁, b₂, hle, hown, body⟩ := h
    refine ⟨P, b₁, b₂, hle, hown, ?_⟩
    simp only [lift] at body ⊢
  ...
  · intro m hv h
    ...
    · rw [bridge_E]; simp only [ValidIndexedPSpPm.PSpace]
      ...
      · rw [Set.preimage_const_of_notMem hv_s, measure_empty]
...
theorem EProdSplit
  ...
  {μ₁ : PMF A} {μ₂ : PMF B}
  :   (fun s => (⟨E₁ s, E₂ s⟩ : A × B))⟨i⟩ ~ PMF.prod μ₁ μ₂
    ⊢ E₁⟨i⟩ ~ μ₁ *' E₂⟨i⟩ ~ μ₂ := by
  sorry
-/
...
noncomputable instance assertionBI : Iris.BI (bProp I Var Val) where
  ...
  persistently_sExists_1 := by
    intro Ψ _ _ h
    obtain ⟨p, hΨp, hp1⟩ := h
    exact ⟨(lift (Ψ p)) ∧' (bpersistently p), ⟨p, rfl⟩, hΨp, hp1⟩
  ...
  later_sExists_false := by
    intro Φ _ _ h
    obtain ⟨p, hΦp, hpm⟩ := h
    exact Or.inr ⟨(lift (Φ p)) ∧' p, ⟨p, rfl⟩, hΦp, hpm⟩
  later_sep := by
    intro _ _
    constructor
    · intro _ _ h; exact h
